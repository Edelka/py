...
class News:
    ...
    def insert(self, dbcon):
        dbcon.cur.execute(f"insert into News values ('{self.text}\', \'{self.city}\', \'{self.date_time}\')")
        print(f"Inserted into News")
        dbcon.connection.commit()
...
class PrivateAdv:
    ...
    def insert(self, dbcon):
        dbcon.cur.execute(f"insert into News values ('{self.text}\', Null, Null)")
        print(f"Inserted adv into News")
        dbcon.connection.commit()
...
class FavoriteBook:
    ...
    def insert(self, dbcon):
        dbcon.cur.execute(f"insert into News values ('{self.text}\', \'{self.author}\', Null)")
        print(f"Inserted fav book into News")
        dbcon.connection.commit()


# selecting output type: 1 - to txt file, 2 - to DB
def output(dbconn, output_type, topic):
    if output_type == '2':
        # dbcon.create_table_news()
        topic.insert(dbconn)
    elif output_type == '1':
...
def publish():
    ...
    output_type = input('Choose how to write data: 1 - to file, 2 - to DB: ')
    dbcon = DBConnection()
    ...
    if input_type == '1':
        try:
            ...
            # topic.write_data()  # write to file
            output(dbcon, output_type, topic)
    ...
    # data from user's input in console
        ...
        try:
            ...
            if n == '1':
                ...
                # write to DB or file
                output(dbcon, output_type, news)
            # topic Private Adv
            elif n == '2':
                private_adv = PrivateAdv(input('Your text: '))
                # write to DB or file
                output(dbcon, output_type, private_adv)
            # topic FavoriteBook
            elif n == '3':
                favorite_book = FavoriteBook(input('Your book: '), input('Author: '))
                output(dbcon, output_type, favorite_book)
            else:
    ...
    # data from json file
        ...
        try:
            path_to_file2 = input('Please provide a path to json file: ')
            ...
            # print(topic_json2)
            # write to DB or file
            output(dbcon, output_type, topic_json2)
...
publish()
print(DBConnection.select(DBConnection(), 'News', '*'))
